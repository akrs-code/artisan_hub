...
export const CartItem = ({ item, onUpdateQuantity, onRemove, onUpdateOptions }) => {
  ...
  return (
    <div className="flex items-start gap-3 sm:gap-4 px-4 sm:px-5 py-4 border-b border-border/60 last:border-b-0 hover:bg-muted/20 transition-colors duration-150 group">

      {/* Product image */}
      <Link
        to={`/product/${item.product._id}`}
        className="w-14 h-14 sm:w-16 sm:h-16 shrink-0 rounded-xl overflow-hidden block bg-muted border border-border/50"
      >
        <img
      ...
      <div className="flex-1 min-w-0">
        <Link
          to={`/shop/${shopId}`}
          className="text-[9px] font-bold font-sans text-primary uppercase tracking-widest mb-0.5 block hover:opacity-75 transition-opacity"
        >
          {shop?.name || 'Artisan Shop'}
        </Link>
        <Link
          to={`/product/${item.product._id}`}
          className="font-headline font-bold text-sm text-foreground hover:text-primary transition-colors line-clamp-1 leading-snug block"
        >
          {item.product.name}
        </Link>
          ...
          <div className="flex flex-wrap gap-1.5 mt-1.5">
            {item.product.colors?.length > 0 && (
              <select
                ...
                className="text-[9px] font-sans px-2 py-1 rounded-lg bg-background border border-border text-muted-foreground hover:text-foreground focus:outline-none focus:border-primary transition-colors cursor-pointer"
              >
      ...
      {/* Line total — desktop */}
      <div className="hidden sm:block w-20 text-right font-headline font-bold text-sm text-foreground shrink-0">
        {formatPrice(item.product.price * item.quantity)}
      </div>
      ...
      <div className="flex justify-center shrink-0 pt-0.5">
        <button
          ...
          className="p-1.5 text-muted-foreground/50 hover:text-destructive hover:bg-destructive/10 rounded-lg transition-all duration-200"
        >

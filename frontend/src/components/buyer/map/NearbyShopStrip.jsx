...
export const NearbyShopStrip = ({ shops, title = 'Nearby Shops' }) => {
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 z-20 pointer-events-none flex flex-col bg-gradient-to-t from-background/30 via-background/10 to-transparent pt-12 pb-6">
        ...
        <div className="flex gap-4 px-6 pb-2 w-max">
          {shops.map((shop) => (
            <Link
              key={shop._id}
              to={`/shop/${shop._id}`}
              className="w-60 bg-card rounded-xl border border-border overflow-hidden hover:border-primary/40 hover:shadow-md transition-all duration-300 group shadow-sm flex flex-col shrink-0"
            >
              {/* Image */}
              <div className="relative h-28 overflow-hidden bg-muted">
                ...
                />
                {/* Rating pill */}
                <div className="absolute top-2 right-2 flex items-center gap-1 bg-card/95 backdrop-blur-sm rounded-full px-1.5 py-0.5 shadow-sm">
                  <Star className="w-2.5 h-2.5 fill-yellow-500 text-yellow-500" />
                  <span className="text-[10px] font-bold font-sans">4.9</span>
                </div>
              </div>
